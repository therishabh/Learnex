...
class Excel extends CI_Controller
{
	...
	function uploadstudent()
	{
		...
		//execute if click on submit button for upload student by excel..
		if($this->input->post('submit-btn'))
		{
			//execute if there is any file is selected..
			if(!empty($_FILES['file']['name']))
			{
				$this->load->helper('oleread');
				...
				$addstudent = 0;
				$updatestudent = 0;
				for($i = 8; $i <= $no_of_rows; $i++)
				{
					if(!empty($excel->sheets[0]['cells'][$i][2]))
					{
						//execute if student email is exist in excel file..
						if(isset($excel->sheets[0]['cells'][$i][18]))
						{
							$email = $excel->sheets[0]['cells'][$i][18];
							$check_email = $this->user_model->check_student_email($email,"student");
							//if email id is already exist into database.. 
							if($check_email)
							{
								$id = $check_email['id'];
								//update data..
								$update_data['name'] = $excel->sheets[0]['cells'][$i][2];
								//execute if gender is selected into excel file..
								if(isset($excel->sheets[0]['cells'][$i][3]))
								$update_data['gender'] = $excel->sheets[0]['cells'][$i][3];
								else
								$update_data['gender'] = "";

								//execute if Date of Birth is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][4]))
								$update_data['dob'] = str_replace("/", "-", $excel->sheets[0]['cells'][$i][4]);
								else
								$update_data['dob'] = "";

								//execute if blood_group is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][5]))
								$update_data['blood_group'] = $excel->sheets[0]['cells'][$i][5];
								else
								$update_data['blood_group'] = "";

								//execute if category is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][6]))
								$update_data['category'] = $excel->sheets[0]['cells'][$i][6];
								else
								$update_data['category'] = "";

								//execute if class is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][7]))
								$update_data['class'] = $excel->sheets[0]['cells'][$i][7];
								else
								$update_data['class'] = "";

								//execute if Date of Joining is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][8]))
								$update_data['doj'] = str_replace("/", "-", $excel->sheets[0]['cells'][$i][8]);
								else
								$update_data['doj'] = "";

								//execute if city is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][9]))
								$update_data['city'] = $excel->sheets[0]['cells'][$i][9];
								else
								$update_data['city'] = "";

								//execute if state is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][10]))
								$update_data['state'] = $excel->sheets[0]['cells'][$i][10];
								else
								$update_data['state'] = "";

								//execute if country is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][11]))
								$update_data['country'] = $excel->sheets[0]['cells'][$i][11];
								else
								$update_data['country'] = "";

								//execute if parent_relation is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][12]))
								$update_data['parent_relation'] = $excel->sheets[0]['cells'][$i][12];
								else
								$update_data['parent_relation'] = "Guardian";

								//execute if parent_name is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][13]))
								$update_data['parent_name'] = $excel->sheets[0]['cells'][$i][13];
								else
								$update_data['parent_name'] = "";

								//execute if parent_occupation is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][14]))
								$update_data['parent_occupation'] = $excel->sheets[0]['cells'][$i][14];
								else
								$update_data['parent_occupation'] = "";

								//execute if parent_phone  is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][15]))
								$update_data['parent_phone'] = $excel->sheets[0]['cells'][$i][15];
								else
								$update_data['parent_phone'] = "";

								//execute if parent_email  is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][16]))
								$update_data['parent_email'] = $excel->sheets[0]['cells'][$i][16];
								else
								$update_data['parent_email'] = "";

								//execute if phone is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][17]))
								$update_data['phone'] = $excel->sheets[0]['cells'][$i][17];
								else
								$update_data['phone'] = "";

								//execute if email address is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][18]))
								$update_data['email'] = $excel->sheets[0]['cells'][$i][18];
								else
								$update_data['email'] = "";

								//execute if Current address is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][19]))
								$update_data['current_address'] = $excel->sheets[0]['cells'][$i][19];
								else
								$update_data['current_address'] = "";


								//execute if permanent address is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][20]))
								$update_data['permanent_address'] = $excel->sheets[0]['cells'][$i][20];
								else
								$update_data['permanent_address'] = "";


								//if course is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][21]))
								{
									$course_name = $excel->sheets[0]['cells'][$i][21];
									$course_detail = $this->user_model->fetchbyfield("name",$course_name,"course");
									$update_data['course'] = $course_detail['id'];
									$update_data['total_amount'] = $course_detail['net_fee_amount'];
								}
								else
								{
									$update_data['course'] = "";
								...
								//if batch is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][22]))
								{
									$batch_name = $excel->sheets[0]['cells'][$i][22];
									$batch_detail = $this->user_model->fetchbyfield("name",$batch_name,"batch");
									$update_data['batch'] = $batch_detail['id'];
								}
								else
								{
									$update_data['batch'] = "";
								...
								//if Discount Amount is not empty in excel file..
								if(isset( $excel->sheets[0]['cells'][$i][24]))
								{
									$insert_data['discount_amount'] = $excel->sheets[0]['cells'][$i][24];	
								...
								//if Net Amount is not empty in excel file..
								if(isset( $excel->sheets[0]['cells'][$i][25]))
								$insert_data['net_amount'] = $excel->sheets[0]['cells'][$i][25];
								else
								$insert_data['net_amount'] = "";

								//if Pay Amount is not empty in excel file..
								if(isset( $excel->sheets[0]['cells'][$i][26]))
								$insert_data['pay_amount'] = $excel->sheets[0]['cells'][$i][26];
								else
								$insert_data['pay_amount'] = "";

								//if Payment Mode is not empty in excel file..
								if(isset( $excel->sheets[0]['cells'][$i][27]))
								$insert_data['payment_mode'] = $excel->sheets[0]['cells'][$i][27];
								else
								$insert_data['payment_mode'] = "";

								//if Cheque Number is not empty in excel file..
								if(isset( $excel->sheets[0]['cells'][$i][28]))
								$insert_data['cheque_number'] = $excel->sheets[0]['cells'][$i][28];
								else
							...
							//if student email id is not exist into database then insert student data into database..
							else
							{
								//fetch last student Data.. for check if there is any student exist or not..
								$student_detail = $this->user_model->fetchlastdata('student');
								//execute if there is any student into database. 
								if($student_detail)
								{

									$insert_data['username'] = increment_string($student_detail['username']);
								}
								//execute if there is no any student into database..
								else
								{
									$insert_data['username'] = "student_1001";
								}
								$insert_data['name'] = $excel->sheets[0]['cells'][$i][2];
								
								$insert_data['password'] = md5('123');
								$insert_data['code'] = $this->user_model->generateRandomString(6);

								if(isset($excel->sheets[0]['cells'][$i][3]))
								$insert_data['gender'] = $excel->sheets[0]['cells'][$i][3];
								else
								$insert_data['gender'] = "";

								//execute if Date of Birth is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][4]))
								$insert_data['dob'] = str_replace("/", "-", $excel->sheets[0]['cells'][$i][4]);
								else
								$insert_data['dob'] = "";
								...
								//execute if blood_group is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][5]))
								$insert_data['blood_group'] = $excel->sheets[0]['cells'][$i][5];
								else
								$insert_data['blood_group'] = "";

								//execute if category is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][6]))
								$insert_data['category'] = $excel->sheets[0]['cells'][$i][6];
								else
								$insert_data['category'] = "";

								//execute if class is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][7]))
								$insert_data['class'] = $excel->sheets[0]['cells'][$i][7];
								else
								$insert_data['class'] = "";

								//execute if Date of Joining is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][8]))
								$insert_data['doj'] = str_replace("/", "-", $excel->sheets[0]['cells'][$i][8]);
								else
								$insert_data['doj'] = "";

								//execute if city is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][9]))
								$insert_data['city'] = $excel->sheets[0]['cells'][$i][9];
								else
								$insert_data['city'] = "";

								//execute if state is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][10]))
								$insert_data['state'] = $excel->sheets[0]['cells'][$i][10];
								else
								$insert_data['state'] = "";

								//execute if country is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][11]))
								$insert_data['country'] = $excel->sheets[0]['cells'][$i][11];
								else
								$insert_data['country'] = "";

								//execute if parent_relation is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][12]))
								$insert_data['parent_relation'] = $excel->sheets[0]['cells'][$i][12];
								else
								$insert_data['parent_relation'] = "Guardian";

								//execute if parent_name is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][13]))
								$insert_data['parent_name'] = $excel->sheets[0]['cells'][$i][13];
								else
								$insert_data['parent_name'] = "";

								//execute if parent_occupation is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][14]))
								$insert_data['parent_occupation'] = $excel->sheets[0]['cells'][$i][14];
								else
								$insert_data['parent_occupation'] = "";

								//execute if parent_phone  is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][15]))
								$insert_data['parent_phone'] = $excel->sheets[0]['cells'][$i][15];
								else
								$insert_data['parent_phone'] = "";

								//execute if parent_email  is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][16]))
								$insert_data['parent_email'] = $excel->sheets[0]['cells'][$i][16];
								else
								$insert_data['parent_email'] = "";

								//execute if phone is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][17]))
								$insert_data['phone'] = $excel->sheets[0]['cells'][$i][17];
								else
								$insert_data['phone'] = "";

								//execute if email address is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][18]))
								$insert_data['email'] = $excel->sheets[0]['cells'][$i][18];
								else
								$insert_data['email'] = "";

								//execute if Current address is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][19]))
								$insert_data['current_address'] = $excel->sheets[0]['cells'][$i][19];
								else
								$insert_data['current_address'] = "";


								//execute if permanent address is filled into excel file..
								if(isset($excel->sheets[0]['cells'][$i][20]))
								$insert_data['permanent_address'] = $excel->sheets[0]['cells'][$i][20];
								else
								$insert_data['permanent_address'] = "";
								...
								if(isset($excel->sheets[0]['cells'][$i][21]))
								{
									$course_name = $excel->sheets[0]['cells'][$i][21];
									$course_detail = $this->user_model->fetchbyfield("name",$course_name,"course");
									$insert_data['course'] = $course_detail['id'];
									$insert_data['total_amount'] = $course_detail['net_fee_amount'];
								}
								else
								{
									$insert_data['course'] = "";
								...
								if(isset($excel->sheets[0]['cells'][$i][22]))
								{
									$batch_name = $excel->sheets[0]['cells'][$i][22];
									$batch_detail = $this->user_model->fetchbyfield("name",$batch_name,"batch");
									$insert_data['batch'] = $batch_detail['id'];
								}
								else
								{
									$insert_data['batch'] = "";
							...
							}//end else condition if student email id is not exist into database then insert student data into database..
						}
						//execute if email id is not filled into excel file.
						else
						{
							//fetch last student Data.. for check if there is any student exist or not..
							$student_detail = $this->user_model->fetchlastdata('student');
				...
				$data['student_detail'] = $this->user_model->fetchalldata("student");

				$data['title'] = "Hello";
				$data['addstudent'] = $addstudent;
				$data['updatestudent'] = $updatestudent;

				$data['page'] = "student-excel-upload";
				$this->load->view('template',$data);
			}
			//execute if there is no file selected.
			else
			{
				header("Location: " . $_SERVER['REQUEST_URI']);
			}
		}
		//execute if there is not clicked in submit button
		else
		{
			//fetch organization details from database..
			$data['organization_detail'] = $this->user_model->get_organization_details();
	...
	function getstudent()
	{
		...
		$student_detail = $this->user_model->fetchstudentforexcel($course,$batch);
