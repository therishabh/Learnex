	...
	<div class="col-lg-6">
		<div class="row">
			<div class="col-lg-4 col-lg-offset-6">
				<a href="<?php echo base_url();?>user/staff"><div class="submit-btn">New Staff</div></a>
			</div>
		</div>
	</div>
</div>
...
<div class="row nav">
	<a href="<?php echo base_url();?>user/staff">
		<div class="col-lg-4 active">Staff</div>
	</a>
			...
			<div class="col-lg-6 left-side">
				...
				<div class="row">
					<div class="col-lg-4 search-bar staff-search">
						<input type="text" class="search-staff staff-name"  placeholder="Staff Name..">
					</div>
					<div class="col-lg-4 search-bar staff-search">
						<select name="" id="" class="staff-type">
			...
			<div class="col-lg-6 right-side">
				...
				<div class="display-div staff">
							...
							<div class="row">
								...
								<div class="col-lg-7">
									<div class="msg success-msg">
										<?php
										if( $this->session->userdata('insert_staff') != "" )
										{
											echo "Staff Has Been Successfully Added.";
										}
										else if( $this->session->userdata('update_staff') != "" )
										{
											echo "Staff Has Been Successfully Updated.";
										}
										?>
									</div>
					...
					<div class="staff-right-div">
						<?php echo form_open_multipart('user/staff','id="staff-form"');?>
						<div class="row">
							<div class="col-lg-5">
								<div class="row">
									<div class="col-lg-12">
								...
								<div class="row" style="margin-top:10px;">
									<div class="col-lg-12">
										...
										<div><input type="text" class="form-control" id="staff-username" name="username" readonly value="<?php echo increment_string($last_staff_username['username']); ?>"></div>
									</div>
								</div>
								<div class="row" style="margin-top:10px;">
									<div class="col-lg-12">
								...
								<div class="row">
									<div class="col-lg-12 image" >
										<div id="uploaded-image">
											<img src="<?php echo base_url(); ?>uploads/staff/default.png"  alt="Staff Image" class="img">
										</div>
										<div class="cross">
						...
						<div class="username" style="display:none;"><?php echo increment_string($last_staff_username['username']); ?></div>
...
jQuery(document).ready(function($) {
	...
	$('.staff-name').keyup(function(event) {
		/* Act on the event */
		var name = $('.staff-name').val()
		var staff_type = $('.staff-type').val();
		var emp_type = $('.staff-emp-type').val();
		$(".la-anim-10").addClass('la-animate');

		$.ajax({
			url: "<?php echo base_url();?>index.php/user/staff_search",
			type: 'POST',
	...
	$('.staff-type').change(function(event) {
		/* Act on the event */
		var name = $('.staff-name').val()
		var staff_type = $('.staff-type').val();
		var emp_type = $('.staff-emp-type').val();
	...
	$('.staff-emp-type').change(function(event) {
		/* Act on the event */
		var name = $('.staff-name').val()
		var staff_type = $('.staff-type').val();
		var emp_type = $('.staff-emp-type').val();
	...
	//execute when click on any staff on left panel for view staff full details..
	$(".staff-grid").on('click', '.bottom-label', function() {
		...
		$(".right-heading").text('View Staff');
		$(".edit .current-staff-id").text(staff_id);
		$(".edit").show();
		$.ajax({
			url: '<?php echo base_url();?>index.php/user/view_staff',
			type: 'POST',
	...
	//execute when user want to edit any staff details.
	//when click on edit button on right panel for edit information of user..
	$('.edit .edit-btn').click(function() {
		var staff_id = $(".edit .current-staff-id").text();
		$(".edit").hide();
		$(".right-heading").text('Edit Staff');
		$(".la-anim-10").addClass('la-animate');

		$.ajax({
			url: '<?php echo base_url();?>index.php/user/edit_staff',
			type: 'POST',
			data: {staff_id: staff_id},
			success: function(result){
				$(".staff-right-div").html(result);
	...
	$(".staff-right-div").on('change', '#files', function(event) {
		...
		{
			$(".cross").hide();
			$("#uploaded-image").html('<img src="<?php echo base_url(); ?>uploads/staff/default.png"  alt="Staff Image" class="img">');
			$("#default_image").val("");
		}
	...
	?>
  	...
  	if( $this->session->userdata('update_staff') )
	{
	?>
		var id = "<?php echo $this->session->userdata('update_staff') ?>";
		$("#"+id).css({
			backgroundColor: 'rgb(142, 213, 114)'
		});
...
<?php
$this->session->unset_userdata('update_staff');
$this->session->unset_userdata('insert_staff');
?>